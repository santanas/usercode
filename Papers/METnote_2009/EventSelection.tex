...
collision events were reconstructed using CMSSW\_3\_3\_6\_patch3
...
then reconstructed using CMSSW\_3\_3\_6\_patch3. The data samples used in
this analysis can be found at:
\begin{itemize}
\item Comparisons with 900~GeV data: /MinBias/Summer09-STARTUP3X\_V8P\_900GeV-v1/GEN-SIM-RECO
\item Comparisons with 2.36~TeV data: /MinBias/Summer09-STARTUP3X\_V8O\_2360GeV-v1/GEN-SIM-RECO
\end{itemize}
...
41). Additionally, any further event selection criteria described in the Section~\ref{sec:event_selection}.
are applied to both data and simulation.

\section{Event Selection}
\label{sec:event_selection}
In order to study the $\etmiss$ performance, an event selection that
removes the instrumental or beam related noise sources is required. In
this section we describe the selection criteria we use to remove these
type of events, and summarize their effect on $\etmiss$ performance.
...
the reconstructed primary vertex properties is implemented. The
selection is based on the following quantities: vertex $\chi^2$, NDF of
vertex, number of tracks which contributed to the vertex reconstruction
with a weight $>0.5$, and vertex $z$ coordinate.

The comparison of the distribution of vertex $z$ coordinate in data
compared to simulation is shown in
Fig.\ref{fig:vertex_selection_1}. Events shown in
Fig.\ref{fig:vertex_selection_1} are required to pass all the selections
described above and additionally to have at least 2 tracks with a
weight~$>0.5$. We implement the official
recommendation from the tracking group to identify collision events by
requiring:
\begin{itemize}
\item vertex $\chi^2!=0$, NDF$!=0$ and contains more than 0 tracks
  attached to it, to reject fake vertices.
...
\begin{eqnarray}
%P &=& \frac{3.8 \times 10^{-3}\frac{\mbox{Hz}}{\mbox{BX}} }{11245.78 \mbox{ Hz}} \\
