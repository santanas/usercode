...
//TFile f_LQ_fit("lq1stgen_M650_fast169_100pb.root");
...
RooRealVar m0("m0","m0",110);
RooRealVar sigma("sigma","sigma",32.3);
RooRealVar alpha("alpha","alpha",-0.35);
RooRealVar n("n","n",29);
  ...
  for(int exp=0;exp<N_exp;exp++)
    {
      ...
      // 	  RooRealVar *N_m0Final_toy = (RooRealVar*)toyMC.fitResult(exp)->floatParsFinal().find("m0");
      // 	  RooRealVar *N_sigmaFinal_toy = (RooRealVar*)toyMC.fitResult(exp)->floatParsFinal().find("sigma");
      // 	  RooRealVar *N_alphaFinal_toy = (RooRealVar*)toyMC.fitResult(exp)->floatParsFinal().find("alpha");
      // 	  RooRealVar *N_nFinal_toy = (RooRealVar*)toyMC.fitResult(exp)->floatParsFinal().find("n");
	 ...
	 && currentSign>=0 && currentSign<10000)
	{
	  ...
	  if(OneGoodExpDone==false)
		...
		= (TH1F*) thisGenData->createHistogram("h_Mej_genData_OneExp", 
	      ...
	      // 		  m0.setVal(N_m0Final_toy->getVal());
	      // 		  sigma.setVal(N_sigmaFinal_toy->getVal());
	      // 		  alpha.setVal(N_alphaFinal_toy->getVal());
	      // 		  n.setVal(N_nFinal_toy->getVal());

	      RooPlot* Mejframe1 = Mej.frame(Name("Mejframe1"),Title("sig + bkg")) ;
	      thisGenData->plotOn(Mejframe1) ;
	      modelFit->plotOn(Mejframe1) ;
	      modelFit->plotOn(Mejframe1, Components(Pdf_bkg), LineStyle(kDashed)) ;
	      ...
	      // 		  m0.setVal(N_m0Final_toy1->getVal());
	      // 		  sigma.setVal(N_sigmaFinal_toy1->getVal());
	      // 		  alpha.setVal(N_alphaFinal_toy1->getVal());
	      // 		  n.setVal(N_nFinal_toy1->getVal());


	      RooPlot* Mejframe2 = Mej.frame(Name("Mejframe2"),Title("only bkg")) ;
	      thisGenData->plotOn(Mejframe2) ;
