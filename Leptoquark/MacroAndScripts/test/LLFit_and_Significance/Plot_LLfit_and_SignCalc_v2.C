{
  ...
  TGraph *g_pullBkg1_mean;
  TGraph *g_pullBkg1_rms;
  ...
  for(int point=0;point<N_point;point++)
    {
      ...
      sprintf(histonameBkg1,"h_Nbkg1_pull_%d",point);
      ...
      TH1F *h_pullBkg1=(TH1F*) inputFile->Get(histonameBkg1);  
      ...
      pullBkg1_mean[point]=h_pullBkg1->GetMean();
      pullBkg1_rms[point]=h_pullBkg1->GetRMS();

      cout << pullLQ_mean[point] << endl;
  ...
  g_pullBkg1_mean = new TGraph(N_point,Lint,pullBkg1_mean);
  g_pullBkg1_rms = new TGraph(N_point,Lint,pullBkg1_rms);
  ...
  g_pullBkg1_mean->Draw("psame");

  TLegend *legend = new TLegend(0.655172,0.684322,0.864943,0.860169);
  legend->SetFillColor(kWhite);
  legend->AddEntry(g_pullLQ_mean,"LQ","p");
  legend->AddEntry(g_pullBkg_mean,"ttbar","p");
  legend->AddEntry(g_pullBkg1_mean,"zjet","p");
  legend->Draw();
