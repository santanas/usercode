...
use Time::Local;
use Getopt::Std;
getopts('d:j:i:c:b:q:s:C:g:w:l:z:nrh');
...
for($i=0; $i<$jobs; $i+=$group) {
	...
	if($tplcfgfile[$j] =~ /\s+untracked\suint32\ssourceSeed\s\=\s123456789/) {
	    $dataset=$datasets[$i+$group-1];
	    chop $dataset;
	    print CFGFILE "untracked uint32 sourceSeed = $seed\n";
	    print "Writing cfg file $cfgfile... \n";
	}
	elsif($tplcfgfile[$j] =~ /untracked\sstring\sfileName\s\=\s\'file\:aodsim\.root/) {
            $nameRoot=$baseName."\_AODSIM_$seed".".root";
...
if(!$opt_n) {
	...
	#print SCRIPTFILE "/bin/cp \*\.CMSSWlog $logdir \n";
        print SCRIPTFILE "/usr/bin/rfcp \*\.CMSSWlog $castorDir \n";
...
sub help(){
    print "Usage: run.pl -c <templateCfg> -w <workdir> -j <njobs> [-i <initialseed>] [-b <basename>] [-s <scratch>] [-C <castorDir>] [-z <maxJobs>] [-n] [-r]\n";
    print "Exemplum: run.pl -c hToWWAnalysis.cfg -w ~/work/HtoWWAnalysis/src -j 100 -i 1000 -b qqH160 -s ~/scratch0 -z 20 -r \n";
    print "Options:\n";
    ...
    print "-s <scratch>:     choose the area where to put the cfg\, csh\, and log files (default is current dir)\n";
    print "-C /castor/...:   choose the CASTOR dir where put the output rootfiles\n";
