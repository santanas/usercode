...
if($opt_S){$digi2rawInputDir=$opt_S;}
else {help();}
...
$digi2rawInputDir = "rfio:"."$digi2rawInputDir";
$logdir = "$logdir";
$confdir = "$confdir";
$scriptdir = "$scriptdir";
$castorDir = "$castorDir";
...
for($i=0; $i<$jobs; $i+=$group) {
    ...
    # write the cfg file
    $cfgfile = $confdir."/".$baseName."\-HLT\-$cfgIndex".".cfg";
	...
	if($tplcfgfile[$j] =~ /\s+\'file\:digi2raw\.root\'/) {
	    $dataset=$datasets[$i+$group-1];
	    chop $dataset;
	    $inputRootFile = $digi2rawInputDir."/".$baseName."\_DIGI2RAW_$seed".".root";
	    print CFGFILE "'file:$inputRootFile'\n";
	    print "Writing cfg file $cfgfile... \n";
	}
	elsif($tplcfgfile[$j] =~ /\s+untracked\sstring\sfileName\s\=\s\"file\:hlt\.root\"/) {
	    $nameRoot=$baseName."\_HLT_$seed".".root";
...
sub help(){
    print "Usage: run.pl -c <templateCfg> -S <dirWhereDIGI2RAW> -w <workdir> -j <njobs> [-i <initialseed>] [-b <basename>] [-s <scratch>] [-C <castorDir>] [-z <maxJobs>] [-n] [-r]\n";
    ...
    print "-S dirWhereDIGI2RAW: choose the directory where the DIGI2RAW Root files are located\n";
