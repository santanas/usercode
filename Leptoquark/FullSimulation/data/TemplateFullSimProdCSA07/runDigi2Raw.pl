...
system("/usr/bin/rfmkdir $castorDir"); 
print " -> created $castorDir\n";

$gensimInputDir = "rfio:"."$gensimInputDir";
$logdir = "$logdir";
$confdir = "$confdir";
$scriptdir = "$scriptdir";
$castorDir = "$castorDir";
...
for($i=0; $i<$jobs; $i+=$group) {
    ...
    # write the cfg file
    $cfgfile = $confdir."/".$baseName."\-DIGI2RAW\-$cfgIndex".".cfg";
	...
	if($tplcfgfile[$j] =~ /\'file\:gensim\.root\'/) {
	    $dataset=$datasets[$i+$group-1];
	    chop $dataset;
	    $inputRootFile = $gensimInputDir."/".$baseName."\_GEN-SIM_$seed".".root";
	    print CFGFILE "'file:$inputRootFile'";
	    print "Writing cfg file $cfgfile... \n";
	}
	elsif($tplcfgfile[$j] =~ /untracked\sstring\sfileName\=\'file\:digi2raw\.root\'/) {
	    $nameRoot=$baseName."\_DIGI2RAW_$seed".".root";
...
sub help(){
    print "Usage: run.pl -c <templateCfg> -S <dirWhereDIGISIM> -w <workdir> -j <njobs> [-i <initialseed>] [-b <basename>] [-s <scratch>] [-C <castorDir>] [-z <maxJobs>] [-n] [-r]\n";
    print "Exemplum: run.pl -c RelVal_Digi_Digi2Raw.cfg -w higgsProd160/src/productionCfgs -j 100 -i 1000 -b qqH160 -s ~/scratch0 -z 20 -r \n";
    print "Options:\n";
    print "-c cfgFile:       choose the template cfg file\n";
    print "-S dirWhereDIGISIM: choose the directory where the DIGI-SIM Root files are located\n";
